...
#define LOG_MODULE_NAME "zephyr_plat"
...
static pthread_attr_t thread_polling_attr;
static uint8_t thread_polling_stack[8192];
...
static pthread_attr_t thread_dispatch_attr;
static uint8_t thread_dispatch_stack[8192];
                   ...
                   Platform_dispatch_indication_f dispatch_indication_f)
{
	...
	int err = pthread_attr_init(&thread_polling_attr);
	err = pthread_attr_setstack(&thread_polling_attr, thread_polling_stack, sizeof(thread_polling_stack));
	err = pthread_create(&thread_polling, &thread_polling_attr, poll_for_indication, NULL);
    if (err != 0)
    {
        LOGE("Cannot create polling thread (err=%i)\n", err);
        goto error3;
    }

    m_dispatch_thread_running = true;
    // Start a thread to dispatch indication
	err = pthread_attr_init(&thread_dispatch_attr);
	err = pthread_attr_setstack(&thread_dispatch_attr, thread_dispatch_stack, sizeof(thread_dispatch_stack));
    if (pthread_create(&thread_dispatch, &thread_dispatch_attr, dispatch_indication, NULL) != 0)
    {
...
error4:
	pthread_cancel(thread_polling);
    // not implemented in zephyr
